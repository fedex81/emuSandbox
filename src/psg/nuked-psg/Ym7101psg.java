...
import static psg.nuked.IYm7101psg.*;

public class Ym7101psg {

    private static final double[] psg_vol = {
            1.0, 0.772, 0.622, 0.485, 0.382, 0.29, 0.229, 0.174, 0.132, 0.096, 0.072, 0.051, 0.034, 0.019, 0.009, 0.0, -1.059
    }; //[17]

   static int[] psg_debug_chan = {0, 2, 1, 3};
    ...
    void PSG_Write(psg_t  chip, int data) { //Bit8u
        ...
        if ((chip.latch & 0x01) > 0) {
            ...
            int channel = chip.latch >> 1; //Bit8u
            if (channel != 3) {
                // Square wave
                if ((data & 0x80) > 0) {
    ...
    double PSG_GetSample(psg_t chip) {
        int i;
        double tsample;
        PSG_UpdateSample(chip);
        if ((chip.debug & 0x01) > 0) {
    ...
    void PSG_Cycle(psg_t  chip) {
        ...
        chip.counter[chip.cycle] = (counter + 1) & 0xFFFF;
